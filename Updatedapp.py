...
ENDPOINT = "https://eastus.api.azureml.ms/api/projects/ridersquery"
...
# --- HTML Placeholder ---
HTML = """<html><body><h2>Agent Chat</h2><ul>{% for msg in chat %}<li><b>{{ msg.role }}:</b> {{ msg.text }}</li>{% endfor %}</ul></body></html>"""
...
def ask():
    ...
    try:
        ...
        for _ in range(30):  # Max 30 attempts (~30 seconds)
            run = project_client.agents.runs.get(thread_id=run.thread_id, run_id=run.id)
            logger.info(f"Run status: {run.status}")
            if run.status.lower() in ["completed", "failed"]:
                break
            time.sleep(1)
